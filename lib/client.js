...
Client.prototype.sftp = function (callback) {
  ...
  ssh.on("keyboard-interactive", function (
    name,
    instructions,
    instructionsLang,
    prompts,
      ...
      if (Buffer.isBuffer(content)) {
        ...
        async.eachSeries(
          ...
          function (pos, callback) {
            ...
            var buf = new Buffer.alloc(chunkSize);
            fs.read(content, buf, 0, chunkSize, lastIndex, function (
              err,
              byteRead,
              buf
            ) {
...
Client.prototype.upload = function (src, dest, callback, mode) {
  dest = unixy(dest);
  ...
  async.waterfall(
    [
      ...
      function (stat, callback) {
        if (stat.isDirectory())
          return callback(new Error("Can not upload a directory"));

        // Get the attributes of the source directory
        fs.stat(path.dirname(src), function (err, dirStat) {
          if (err) return callback(err);
          self.mkdir(path.dirname(dest), dirStat, function (err) {
            callback(err, stat);
          });
        });
      },
        ...
        self.write(
          {
            ...
            destination: dest,
            content: fd,
            attrs: { ...stat, mode: mode ? mode : stat.mode },
          },
          callback
        );
      },
...
Client.prototype.download = function (src, dest, callback, mode) {
  var self = this;
  ...
  self.sftp(function (err, sftp) {
    ...
    sftp_readStream
      .pipe(fs.createWriteStream(dest, mode ? { mode } : undefined))
      .on("close", function () {
        self.emit("read", src);
        callback(null);
      })
